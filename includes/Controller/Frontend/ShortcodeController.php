...
class ShortcodeController
{
    ...
    public static function renderMenu($atts = []): string
    {
        ...
        $atts = shortcode_atts([
            ...
            'title' => __('Today\'s Menu', DMM_TEXT_DOMAIN),
            'show_prices' => 'yes',
            //TOOD: Implentieren
                    ...
                    <div class="menu-layout">
                                    ...
                                    <?php foreach ($items as $item): ?>
                                        ...
                                        <div class="menu-item" data-item-available_quantity="<?php echo esc_attr($item->available_quantity); ?>" data-item-id="<?php echo esc_attr($item->id); ?>">
                                            <div class="menu-item-header">
                                                ...
                                                <?php else: ?>
                                                    <span class="menu-item-title"><?php echo esc_html($item->title); ?> (<?php echo sprintf(__('%dx available', DMM_TEXT_DOMAIN), esc_html($item->available_quantity)); ?>)</span>
                                                <?php endif; ?>
                                            ...
                                            <?php
                                                ...
                                                foreach ($props as &$prop) {
                                                    echo "<div style=\"background-color: $main_color;\" class=\"badge text-decoration-none me-1 mb-1\">" . __($prop, DMM_TEXT_DOMAIN) . "</div>";
                                                }
                                            ?>
                        ...
                        <div class="order-info-column">
                            <div class="order-summary">
                                <h3><?php _e('Order Summary', 'daily-dish-manager'); ?></h3>
                                <div class="order-total">
                                    <?php _e('Total:', 'daily-dish-manager'); ?>
                            ...
                            <div class="customer-info">
                                <div class="form-field">
                                    <label for="customer_name">
                                        <?php _e('Name', 'daily-dish-manager'); ?>*
                                    </label>
                                    <input type="text" name="customer_name" id="customer_name" required>
                                </div>
                                <div class="form-field">
                                    <label for="customer_phone">
                                        <?php _e('Phone Number', 'daily-dish-manager'); ?>
                                        <?php _e('(for possible inquiries)', 'daily-dish-manager'); ?>
                                    </label>
                                    <input type="tel"
                                        name="customer_phone"
                                        id="customer_phone"
                                        pattern="[0-9\s\+\-()]+"
                                        placeholder="<?php _e('e.g. (555) 123-4567', 'daily-dish-manager'); ?>">
                                </div>

                                <div class="form-field">
                                    <label for="consumption_type">
                                        <?php _e('Pick up or eat in', 'daily-dish-manager'); ?>*
                                    </label>
                                    <select name="consumption_type" id="consumption_type" required>
                                        <!-- TODO: Let user choose -->
                                        <option value=""><?php _e('Please choose', 'daily-dish-manager'); ?></option>
                                ...
                                <div class="form-field">
                                    <label for="pickup_time">
                                        <?php _e('Pickup Time', 'daily-dish-manager'); ?>*
                                ...
                                <div class="form-field">
                                    <label for="general_notes">
                                        <?php _e('Order notes', 'daily-dish-manager'); ?>
                                    </label>
                            ...
                            <button type="submit" class="submit-order" style="background-color: <?php echo SettingsController::getMainColor(); ?>;">
                                <?php _e('Place Order', 'daily-dish-manager'); ?>
                            </button>
                        </div>
                    </div>
                </form>
