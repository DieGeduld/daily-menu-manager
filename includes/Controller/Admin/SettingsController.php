...
class SettingsController
{
    ...
    public static function displaySettingsPage()
    {
        ...
        if (isset($_POST['save_menu_settings']) && check_admin_referer('daily_dish_settings_nonce')) {

            /* Properties */
            $properties = isset($_POST['daily_dish_properties']) ? $_POST['daily_dish_properties'] : [];
            ...
            /* Order Times */
            if (isset($_POST['daily_dish_order_times'])) {
                $order_times = $_POST['daily_dish_order_times'];
            ...
            /* Main Color */
            if (isset($_POST['daily_dish_main_color'])) {
                if (function_exists('sanitize_hex_color')) {
                    $main_color = sanitize_hex_color($_POST['daily_dish_main_color']);
                } else {
                    $main_color = sanitize_text_field($_POST['daily_dish_main_color']);
                }
            ...
            /* Currency */
            if (isset($_POST['daily_dish_currency'])) {
                $currency = sanitize_text_field($_POST['daily_dish_currency']);
                $settings_model->set('currency', $currency);

                if ($currency === 'custom' && isset($_POST['daily_dish_custom_currency_symbol'])) {
                    $custom_currency_symbol = sanitize_text_field($_POST['daily_dish_custom_currency_symbol']);
                    $settings_model->set('custom_currency_symbol', $custom_currency_symbol);
                }
            }

            /* Price Format */
            if (isset($_POST['daily_dish_price_format'])) {
                $price_format = sanitize_text_field($_POST['daily_dish_price_format']);
                $settings_model->set('price_format', $price_format);
            }

            /* Time Format */
            if (isset($_POST['daily_dish_time_format'])) {
                $time_format = sanitize_text_field($_POST['daily_dish_time_format']);
                $settings_model->set('time_format', $time_format);
            }

            /* Order Prefix */
            $consumption_types = isset($_POST['daily_dish_consumption_types']) ? $_POST['daily_dish_consumption_types'] : [];
            ...
            /* Order Prefix */
            $menu_types_labels = isset($_POST['daily_dish_types_labels']) ? $_POST['daily_dish_types_labels'] : [];
            $menu_types_plurals = isset($_POST['daily_dish_types_plurals']) ? $_POST['daily_dish_types_plurals'] : [];
        ...
        if (isset($_POST['run_migrations']) && check_admin_referer('daily_dish_settings_nonce')) {
            try {
