jQuery(document).ready(function ($) {
  ...
  $('.menu-items').sortable({
    ...
    start: function (e, ui) {
      ...
      ui.helper.addClass('is-dragging').css('box-shadow', '0 2px 8px rgba(0,0,0,0.1)');
    },
    ...
    stop: function (e, ui) {
      ui.item.removeClass('is-dragging').css('box-shadow', '');
    },
  ...
  $('.add-menu-item').on('click', function () {
    ...
    // Type aktualisieren
    $newItem.attr('data-type', type).find('input[name*="[type]"]').val(type);
            ...
            if (response.success) {
              ...
              if (type == 'to') {
                window.location.href =
                  window.location.pathname + '?page=daily-dish-manager&menu_date=' + selectedDate;
              } else {
                window.location.href =
                  window.location.pathname + '?page=daily-dish-manager&menu_date=' + currentDate;
              }
            } else {
              showFeedback(
                response.data.message || window.dailyMenuAdmin.messages.copyError,
                'error',
              );
            }
          },
  ...
  const mainPickr = flatpickr('.flatpickr-wrapper', {
    ...
    onDayCreate: function (dObj, dStr, fp, dayElem) {
      try {
        ...
        if (
          window.dailyMenuAdmin &&
          window.dailyMenuAdmin.menus &&
