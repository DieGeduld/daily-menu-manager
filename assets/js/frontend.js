jQuery(document).ready(function($) {
    ...
    function updateTotal() {
        ...
        $('#total-amount').text(total.toFixed(2) + ' ' + window.dailyMenuAjax.currencySymbol);
    }
                ...
                if (response.success) {
                    ...
                    response.data.items.forEach(function(item) {
                        detailsHtml += `<li style="margin-bottom: 10px;">
                            ${item.quantity}x ${item.title} (${(item.price * item.quantity).toFixed(2)} ${window.dailyMenuAjax.currencySymbol})`;
                        if (item.notes) {
                    ...
                    detailsHtml += '</ul>';
                    detailsHtml += `<p><strong>Gesamtbetrag: ${response.data.total_amount.toFixed(2)}&nbsp;${window.dailyMenuAjax.currencySymbol}</strong></p>`;
